/**
 * Parent-wallet page with cosmos-kit and wagmi (Next.js App Router): the widget
 * reuses both the Cosmos wallet (cosmos-kit) and the EVM wallet (wagmi) already
 ...
 *
 * cosmos-kit and wagmi are client-only, so this page is a client component and
 * the widget is dynamically imported with ssr:false to keep iframe-only code out
...
import { fromCosmosKit, fromWagmi } from '@atom-circuit/embed-sdk';
...
// instead of as a raw overlay. This is a client component, so the import lives
// here; you can also place it in your root layout. You can theme or replace it
// with your own styles if you want the picker to match your brand.
...
const evmChains = [mainnet, base, arbitrum, optimism, polygon, avalanche];
  ...
  useEffect(() => {
    setCosmos(
      isWalletConnected && cosmosClient
        ? fromCosmosKit(cosmosClient, { metadata: { name: 'My App' } })
        : undefined,
    );
  }, [isWalletConnected, cosmosClient]);
